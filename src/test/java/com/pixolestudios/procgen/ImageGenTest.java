...
public class ImageGenTest {
    ...
    public void assertImageResolutionAsExpected() {
        ...
        ImageGen img = new ImageGen(mapToImage, testOutLoc + "/image.png", pixelsPerGridPoint);
        img.GenerateImg();

        BufferedImage bimg = null;
        try {
            bimg = ImageIO.read(new File(testOutLoc + "/image.png"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void imageGenOnUninitiatedMapDoesntError() {
        int mapXSize = 10, mapYSize = 20;
        MapGrid mapToImage = new MapGrid(mapXSize, mapYSize);

        ImageGen img = new ImageGen(mapToImage, testOutLoc + "/image.png", 1);
        img.GenerateImg();
    }
}